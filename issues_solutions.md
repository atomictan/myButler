...
- **Issue (2026-08-31):** Installing the updated icon build to a connected iPhone from the CLI is blocked because Xcode reports the device is unpaired.
...
- **Issue (2026-09-01):** Public-readiness audit found tracked credential/internal-development artifacts, including a Doubao user-info CSV with an access key pair, local Doubao example scripts, a hardcoded realtime App Key, Python caches, audio blobs, and Xcode user state.
  - **Solution:** Removed those artifacts from the current tree, added ignore rules for credentials/generated files/local logs, moved the realtime App Key out of source and into Settings, rewrote reachable Git history, and kept credential rotation as a required safety step before making the existing GitHub repo public.
